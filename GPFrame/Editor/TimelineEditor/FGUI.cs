...
namespace GPEditor
{
	...
	//	public const float TIMELINE_TENTH_SECOND_TICK_SIZE = 5;
		...
		public static readonly Color EVENT_COLOR_PRO = new Color( 0.24f, 0.36f, 0.60f, 1f );

		public static readonly Color EVENT_COLOR = new Color( 0.24f, 0.36f, 0.60f, 1f );

		public static readonly Color TEXT_COLOR_PRO = new Color( 0.8f, 0.8f, 0.8f, 1f );
...
//				GUI.color = Color.white;
	...
	//	public static float CalcTimelineHeight( Timeline timeline )
	//	{
	//		TimelineEvent[] events = timeline.events;
	//		
	//		if( events.Length == 0 )
	//		{
	...
	//		
	//		
	//		int maxSimultaneousEvents = 1;
	//		
	//		
	//		for( int i = 0; i != events.Length; ++i )
	//		{
	//			int simultaneousEvents = 1;
	//			float currentEvtTriggerTime = events[i].triggerTime;
	//			float currentEvtEndtime = events[0].triggerTime+events[0].duration;
	//			
	...
	//				}
	//				
	//				if( events[j].triggerTime > currentEvtEndtime || events[j].triggerTime+events[j].duration < currentEvtTriggerTime )
	//				{
	//					continue;
	//				}
	//				
	...
	//			}
	//			
	//			if( maxSimultaneousEvents < simultaneousEvents )
	//			{
	//				maxSimultaneousEvents = simultaneousEvents;
	//			}
	//		}
	//		return maxSimultaneousEvents*TIMELINE_HEIGHT;
	//	}
		
	/*	public static Rect HorizontalPanelSeparator( Rect rect )
		{
			GUIStyle panelSeparator = GUI.skin.GetStyle("HorizontalPanelSeparator");
			GUIStyle panelSeparatorHandle = GUI.skin.GetStyle("HorizontalPanelSeparatorHandle");
			
			int guiID = EditorGUIUtility.GetControlID( FocusType.Passive );
			...
			case EventType.Repaint:
				...
				panelSeparator.Draw( rect, empty, 0 );
				
				Rect handleRect = rect;
				handleRect.yMin = ((rect.yMax - rect.yMin)*0.5f)+rect.yMin-(panelSeparatorHandle.fixedHeight*0.5f);
				handleRect.yMax = handleRect.yMin+panelSeparatorHandle.fixedHeight;
	//			Debug.Log( rect + " | " + handleRect );
				panelSeparatorHandle.Draw( handleRect, empty, 0 );
				break;
			...
			case EventType.MouseDrag:
				if( EditorGUIUtility.hotControl == guiID )
				{
	//				float x = Event.current.mousePosition.x - oldPos.x;
	//				float y = Event.current.mousePosition.y - oldPos.y;
	//				oldPos = Event.current.mousePosition;
	//				rect.x += Event.current.delta.x;
					rect.x = Event.current.mousePosition.x-panelSeparator.fixedWidth*0.5f;
	//				if( rect.x < 200 )
	//				{
	//					rect.x = 200;
	//				}
	//				Debug.Log( string.Format("Mouse: ({0},{1}) | Delta: ({2},{3})", x, y, Event.current.delta.x, Event.current.delta.y ) );
					Event.current.Use();
				}
				break;
			}
