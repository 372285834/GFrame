...
/*		DebugHelper.Assert(!Singleton<BattleLogic>.instance.isFighting || Singleton<GameLogic>.instance.bInLogicTick || Singleton<FrameSynchr>.instance.isCmdExecuting);*/
	...
	public static VCollisionShape InitActorCollision(Actor actor)
	{
		if (actor.shape == null)
		{
			SCollisionComponent sCollisionComponent = null;
			if (!VCollisionShape.s_componentCache.TryGetValue(actor.transform.GetInstanceID(), out sCollisionComponent))
			{
				sCollisionComponent = actor.transform.GetComponent<SCollisionComponent>();
				VCollisionShape.s_componentCache[actor.transform.GetInstanceID()] = sCollisionComponent;
			}
...
// 			}
			...
			{
				vCollisionShape = VCollisionShape.createFromCollider(actor.transform.gameObject);
			}
	...
	public static VCollisionShape InitActorCollision(Actor actor, GameObject gameObj, string actionName)
	{
		VCollisionShape vCollisionShape = null;
		if (actor.shape == null)
		{
			SCollisionComponent sCollisionComponent = null;
			if (null != gameObj && !VCollisionShape.s_componentCache.TryGetValue(actor.transform.GetInstanceID(), out sCollisionComponent))
			{
...
// 			}
    ...
    public virtual void Born(Actor actor)
    {
        actor.shape = this;
        this.owner = actor;
    }
