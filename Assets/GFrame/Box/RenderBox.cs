...
public class RenderBox : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (mRender == null)
        {
            ...
            if (mRender == null)
                return;
            sMat = mRender.sharedMaterial;
            this.enabled = false;
        }
        canDissolve = mRender != null && sMat != null && mRender.enabled;
    }
    ...
    public void DissolveStart(float p=0f)
    {
        ...
        {
            progress = p;
            //if (dissMat == null && SRPSetting.Inst != null)
            //    dissMat = new Material(SRPSetting.Inst.DissolveMatInst);
            dissMat.mainTexture = sMat.mainTexture;
            mRender.material = dissMat;
            this.enabled = true;
        }
    }
    ...
    {
        if (this.enabled && dissMat != null)
        {
            dissMat.SetFloat("_DissolveThreshold", progress);
        }
    }
    ...
    public static void SetPropertyBlockAlpha(Renderer render, float a)
    {
        ...
        render.shadowCastingMode = isAlpha ? UnityEngine.Rendering.ShadowCastingMode.Off : UnityEngine.Rendering.ShadowCastingMode.On;
