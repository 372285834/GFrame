...
public abstract class DecoratorEditor : Editor
{
    ...
    public static Type GetAssembly(string editorTypeName)
    {
        var tt = AppDomain.CurrentDomain.GetAssemblies();
        for (int iLoop = 0; iLoop < tt.Length; ++iLoop)
        {
            if (tt[iLoop].FullName.Contains("UnityEditor"))
            {
                var t = tt[iLoop].GetType(editorTypeName);
                if (t != null)
                {
                    return t;
                }
    ...
    {
        this.decoratedEditorType = GetAssembly(editorTypeName);
        //this.decoratedEditorType = editorAssembly.GetTypes().Where(t => t.FullName == editorTypeName).FirstOrDefault();
