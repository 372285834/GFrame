...
//			return (range.start > start && range.start < end) || (range.end > start && range.end < end );
	...
	public class FEvent : FObject
	{
		...
		[SerializeField]
		[HideInInspector]
		private bool _triggerOnSkip = false;
		...
		public FrameRange FrameRange { get { return _frameRange; } 
			set { 
				FrameRange oldRange = _frameRange;
				_frameRange = value;
				OnFrameRangeChanged( oldRange );
			} 
		}
		...
		/**
		 * @brief Used to setup your code that gets called when the event updates in editor mode, e.g. flux window.
		 * @param framesSinceTrigger How many frames passed since TriggerFrame
		 * @param timeSinceTrigger How much time passed since TriggerFrame
		 */ 
		...
		{
			return GetId() == _track.GetEvents().Count-1;
		}
		...
		public FrameRange GetMaxFrameRange()
		{
			...
			{
				range.Start = _track.GetEvents()[id-1].End;
			}

			if( id == _track.GetEvents().Count-1 ) // last one?
			{
				range.End = _track.GetTimeline().Sequence.Length;
			}
			else
			{
				range.End = _track.GetEvents()[id+1].Start;
			}
