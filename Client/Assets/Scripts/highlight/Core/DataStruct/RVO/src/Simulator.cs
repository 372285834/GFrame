...
namespace RVO
{
    ...
    public class Simulator
    {
      ...
      //  internal IDictionary<int, int> index2agentNoDict_;
         ...
         * agent.</param>
         */
        public int addAgent(int id,Vector2 position)
        {
         ...
         * this agent.</param>
         */
        public int addAgent(int id,Vector2 position, float neighborDist, int maxNeighbors, float timeHorizon, float timeHorizonObst, float radius, float maxSpeed, Vector2 velocity)
        {
            Agent agent = new Agent();
            agent.id_ = id;
          //  s_totalID++;
        ...
        public void getPosNeighbors(List<KeyValuePair<float, int>> list, Vector2 pos, float rang)
        {
            rang = RVOMath.sqr(rang);
            Simulator.Instance.kdTree_.computePosNeighbors(list, pos, ref rang);
        }
        /**
        ...
        {
            if (RVOMath.absSq(prefVelocity) > 1.0f)
            {
                prefVelocity = RVOMath.normalize(prefVelocity);
            }
