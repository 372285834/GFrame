...
namespace RVO
{
    ...
    internal class KdTree
    {
        ...
        static Agent neighborsAgent = new Agent();
        internal void computePosNeighbors(List<KeyValuePair<float, int>> list, Vector2 pos, ref float rangeSq)
        {
            list.Clear();
            neighborsAgent.position_ = pos;
            neighborsAgent.agentNeighbors_.Clear();
            neighborsAgent.maxNeighbors_ = int.MaxValue;
            queryAgentTreeRecursive(neighborsAgent, ref rangeSq, 0);
            for(int i=0;i< neighborsAgent.agentNeighbors_.Count;i++)
            {
                list.Add(new KeyValuePair<float, int>(neighborsAgent.agentNeighbors_[i].Key, neighborsAgent.agentNeighbors_[i].Value.id_));
            }
        }
        /**
