...
namespace DataStructure
{
    ...
    public class QuadTree<T>
    {
        ...
        public QuadTree(int splitCount, ref Quad region)
        {
            ...
            root = CreateBranch(this, null, ref region,0);
        }
        /// <summary>
        /// Creates a new QuadTree.
        /// </summary>
        ...
        public bool Remove(T value)
        {
            ...
            if (leafLookup.TryGetValue(value, out leaf))
            {
                ...
                leaf.Branch.Leaves.Remove(leaf);
                leafPool.Push(leaf);
                leaf.Branch = null;
        ...
        public bool IsChange(T value,Quad quad)
        {
            bool b = false;
            Leaf leaf = null;
            if (leafLookup.TryGetValue(value, out leaf))
            {
                b = leaf.IsChange(ref quad);
            }
            return b;
        }
        /// <summary>
        ...
        static Branch CreateBranch(QuadTree<T> tree, Branch parent, ref Quad quad,byte _idx)
        {
        ...
        internal class Branch
        {
            ...
            internal void Insert(Leaf leaf)
            {
                ...
                if (Split)
                {
                    for (byte i = 0; i < 4; ++i)
                    {
                        ...
                        {
                            if (Branches[i] == null)
                                Branches[i] = CreateBranch(Tree, this, ref Quads[i],i);
            ...
            public bool IsInner(ref Quad _quad)
            {
                if (this.Parent != null)
                {
                    Quad quad = this.Parent.Quads[this.idx];
                    return quad.Contains(ref _quad);
                }
                return true;
            }
        }
        ...
        internal class Leaf
        {
            ...
            internal T Value;
            internal Quad Quad;
            public bool IsChange(ref Quad quad)
            {
                return !this.Branch.IsInner(ref quad);
            }
        }
    }
