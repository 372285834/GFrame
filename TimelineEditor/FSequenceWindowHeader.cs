...
namespace GPEditor
{
	public class FSequenceWindowHeader
	{
		...
		private GTimelineEditor[] _sequences;
		...
		{
			_sequences = GameObject.FindObjectsOfType<GTimelineEditor>();
			System.Array.Sort<GTimelineEditor>( _sequences, delegate(GTimelineEditor x, GTimelineEditor y) { return x.name.CompareTo(y.name); } );
			
			_sequenceNames = new GUIContent[_sequences.Length];
...
//			Debug.Log( _sequenceNameRect.width );
			...
			_lengthFieldRect.x = rect.xMax - _lengthFieldRect.width;
